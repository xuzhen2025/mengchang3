...
export default function MaterialsView({ onTriggerTask, onNavigateToDelivery, onDetailStateChange, initialSearch }: MaterialsViewProps) {
  ...
  const handleBatchOptionSelect = (category: string, option: string) => {
    ...
    if (option === "放入回收站") {
      if (!window.confirm(`删除后将把选中的 ${count} 个素材移入回收站，可在回收站恢复。确认继续吗？`)) return;
  ...
  const handleDeleteVideo = (id: string) => {
    if (confirm("删除后将把该素材移入回收站，可在回收站恢复。确认继续吗？")) {
